...
@cython.nonecheck(False)
@cython.cdivision(True)
cpdef global_sdp_clf_p(double[:, :] X, long[:] fX,
            long[:] y_pred, double[:, :] data,
            double[:, :, :] values, double[:, :, :, :] partition_leaves_trees,
            long[:, :] leaf_idx_trees, long[:] leaves_nb, double[:] scaling, list C, double global_proba,
            int num_threads):
...
cpdef global_sdp_clf_ptrees(double[:, :] X, long[:] fX,
    ...
    #                         print('debug', j)
            ...
            for i in range(N):
                for b in range(n_trees):
                    for leaf_numb in range(leaves_nb[b]):
                        mean_forest[R_buf[i], 0] += mean_forest_b[b, leaf_numb, R_buf[i], 0]
                        mean_forest[R_buf[i], 1] += mean_forest_b[b, leaf_numb, R_buf[i], 1]
                        mean_forest[R_buf[i], 2] += mean_forest_b[b, leaf_numb, R_buf[i], 2]
...
@cython.nonecheck(False)
@cython.cdivision(True)
cpdef global_sdp_clf_p3(double[:, :] X, long[:] fX,
            long[:] y_pred, double[:, :] data,
            double[:, :, :] values, double[:, :, :, :] partition_leaves_trees,
            long[:, :] leaf_idx_trees, long[:] leaves_nb, double[:] scaling, list C, double global_proba,
            int num_threads):
