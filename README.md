...
The Agnostic approaches permit to explain any data (**X**, **Y**) or model (**X**, **f(X)**) 
...
```python
sufficient_coal, sdp_coal, sdp_global = acv_xplainer.sufficient_coal_rf(X_test, y_test, X_train, y_train, pi_level=pi_level,
                                                                    classifier=int(True))
```
...
apparition of the given variable in the Sufficient Explanations. 

* **How to compute the Local Explanatory Importance ?**

```python
lxm = acv_xplainer.compute_local_sdp(X_train.shape[1], sufficient_coal)
```

#### Local Rule-based Explanations

* **How to compute the Local Rules Explanations ?**
...
provides the following different Shapley Values: 
...
# cat_index := list[list[int]] that contains the index of the dummies or one-hot variables grouped 
...
```
#### **How to compute <img src="https://latex.codecogs.com/gif.latex?SDP_S%28x%2C%20f%29" />  for tree-based model?**

```python
sdp = acvtree.compute_sdp_clf(X, S, data_bground)
```
#### **How to compute the Sufficient Coalition <img src="https://latex.codecogs.com/gif.latex?S^\star" /> and the Global SDP importance for tree-based model ?**
```python 
